...
module CRISP
  module Bootstrap
    # Information on how a particular program was or will be invoked.
    class ProgramInvocationInfo
      @program = nil
      @mode = nil
      @arguments = nil
      @parameters = nil
      ...
      def initialize(program, mode, arguments, parameters)
        @program = program
        @mode = mode
        @arguments = arguments
      ...
      def exec!
        Process.exec(@program.binary, *@arguments)
      end
      ...
      # def marshal_dump
      #   { :program => @program.uuid, :mode => @mode.name, :arguments => @arguments,
      #     :parameters => @parameters }
      # end

      # def marshal_load(hsh)
      #   @program = Program.from_uuid(hsh[:program])
      #   @mode = @program.modes[hsh[:mode]]
      #   @arguments = hsh[:arguments]
      #   @parameters = hsh[:parameters]
      # end
